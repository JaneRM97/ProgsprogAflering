...
object Lambda {
  ...
  def encode(e: Exp): Exp =
    e match {
      case IntLit(c) if c >= 0 => // non-negative integer literals are encoded as on slide 18
        def help(c: Int): Exp = {
          if (c == 0) VarExp("z") else CallExp(VarExp("s"), List(help(c-1)))
        }
        LambdaExp(List(FunParam("s", None)), LambdaExp(List(FunParam("z", None)), help(c)))
      ...
      case BinOpExp(leftexp, EqualBinOp(), IntLit(0)) => // e == 0, slide 18
       CallExp(CallExp(encode(leftexp), List(LambdaExp(List(FunParam("n", None)), encode(BoolLit(false))))), List(encode(BoolLit(true))))
  ...
  def decodeNumber(v: Val): Int = v match {
    case ClosureVal(params, _, exp, env, defs) =>
